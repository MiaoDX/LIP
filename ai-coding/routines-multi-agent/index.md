# 一个人 + 多个 Routine：让 Claude Code 帮我们把想法快速完成 0 到 1
...
**但初版 plan 离能拆 issue 还差很远**。接下来会围绕这份 plan 反复聊——挑战它的技术选型、质疑某些步骤的必要性、让它解释为什么某个方案比另一个更合适、补充一些它漏掉的边界情况。这个讨论过程往往要来回很多轮，plan 会被不断推翻、重写、细化。
...
![四个 routine 的架构总览 - GitHub 居中作为协作基底](images/09-architecture-overview.svg)
...
![三态自评决策流 - 给 agent "做一半也 OK" 的出口](images/10-three-state-self-eval.svg)
...
![两条协作协议：分支命名空间隔离（上）+ GitHub comment 作为消息总线（下）](images/11-message-protocol.svg)
...
**一个意外的副产品**：因为 routine 都跑在云端、完全不依赖本地电脑开不开，这套流程会把订阅额度拉得比以前满得多。以前是"坐在电脑前才会消耗 token"，现在是 auto_pr 每小时自己跑、daily routines 每天定时醒来，配额几乎没有闲着的时候。

![Max 20x 的 All models 周配额 100% 用满 —— 以前从来没这么整齐的跑满过](images/12-max-quota-used.png)

这不算什么值得夸耀的成就，但确实是一个之前没预料到的变化：**agent 的工作节奏和人的作息解耦之后，订阅的"实际使用率"会显著上升**。用 Max 20x 的朋友可以留意一下这个现象。
