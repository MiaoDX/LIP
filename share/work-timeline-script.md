...
我的核心专长是系统架构和算法工程。这次主要看两类机会：一类是机器人系统工程，另一类是数据和评测。我不限定某个具体 JD，更关注团队正在解决什么问题，以及我能否继续从问题定义、方案推进一直跟到真机验证和交付。
...
这一阶段我主责模型真机部署、数据生产与批量评测，模型同事主责架构和训练。我们通过评测和真机失败证据共同收敛方案：判断下一轮应该补数据、改模型，还是修复环境与工程链路。我也会把部署约束带回方案讨论，例如自回归模型的输出表示会影响序列长度和真机耗时。2026 年两项具体 π0.5 任务微调属于另一个明确范围，不能把两个阶段的 ownership 混在一起。
...
下一步主要看机器人系统工程和数据评测两类机会。我不限定某个具体 JD，希望继续深入一线技术，从问题定义、方案推进一直跟到真机验证和交付；具体工作可以结合团队当前最需要解决的问题形成。
